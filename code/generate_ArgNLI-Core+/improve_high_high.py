...
high_high_system_prompt = """Generate a statement that argumentatively supports the given statement but is hard to infer from the given statement. 
Use as many words as possible from the given statement to generate your statement but keep the statement grammatical!
For example: 'John likes to eat bananas.' -> 'Someone likes to eat bananas and knows John.' or
'The moon is made of green cheese.' -> 'Therefore, we should make green cheese fondue from the moon.' or
'The constitution is designed to give freedom to everone' -> 'Some freedoms are designed to give everyone a constitution.'
It is often a useful strategy to reverse the logic of the statement and potentially use helpers like 'someone' or 'something'. 
Keep the agent and patiens roles adapted to the content.
The output MUST be grammatical and MUST make somewhat sense.
The output MUST NOT paraphrase the input statement!
If you cannot find something that is suitable, output "Not found".
Don't add prefixes, notes, or explanations to the output.
Statement:"""
...
def generate_with_params(prompt, target_sim="low", target_dist = "low", model='llama3.1:70b',):
    ...
    if target_sim == "low" and target_dist == "low":
        prompt = low_low_system_prompt + prompt
        options={
        ...
        prompt = high_high_system_prompt + prompt
        options={
...
def process_chunk(chunk_df, model, output_path, append_mode=False):
    ...
    #     "high_high_consequent_new": high_high
    # })
